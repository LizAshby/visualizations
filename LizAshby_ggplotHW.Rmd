...
g + geom_line() #Download ggplot, then try running these
...
#sometimes order of coding makes a difference. Not so much for theme, but add it so it makes sense
```
...
g + geom_point() + # plot our original x, y data
  geom_point(aes(x, y2)) + # co-plot a second variable. Here we're assigning by posision. You could swap x and y by making it an argument. ex: (y = x, x = y2)
  #Can also add a function for a trnasformed variable. ex: z <- x^3. Can then include z as a variable, and it will graph it
  ggtitle("Our First Amazing Plot\nNow with TWO Variables!") +     #\n puts next sec on 2nd line
  xlab("Shared X Coordinates") +
  ylab("Y Label") + 
...
g + geom_point() +
  ...
  geom_line(aes(x, y2), linetype="dashed", size=1.5, alpha=0.6) + #add a transparent dashed line to the y2 line. Alpha is transparency value on scale of 1. Closer to 1, more transparent
  ggtitle("Our First Amazing Plot\nNow with TWO Variables!") +
  xlab("Shared X Coordinates") +
  ylab("Y Label") + 
  theme_classic()
...
g + geom_point(aes(color=y2, size=y2, fill=y2), shape=24) +
  ...
  scale_color_gradient(low="red", high = "yellow", name = "Color") + #change the color of the shapes. Color and fil can be different if the shape is hollow
  ...
  ggtitle("Our First Amazing Plot\nNow with TWO Variables!") + 
  theme_dark() +
  theme(legend.box = "horizontal")  # Add theme color before legend, because legend is part of theme
...
Can only hae one color schemee per plot

Can include color, shape to denote sex, other variables in data points
...
```{r}
g + geom_point(aes(color = y2, size = y2, fill = y2), shape = 18) +
  geom_point(aes(x = x, y = y2, color = y2), size = 4, shape = 16) + #we can co-plot a second variable with same colors
  scale_color_gradient(low="red", high = "yellow", name = "Color") + #change the color of the shapes
  scale_fill_gradient(low="yellow", high="red", name = "Fill") + #change the color of the fill
  geom_line(aes(x, y2), color = "red", linetype = "dashed", size = .8, alpha=.8) + #add a transparent dashed line
  scale_color_gradient(low = "red", high = "yellow", name = "Line") + #replaces the legend item
  xlab("Shared X Coordinates") +
  ylab("Y Label") +
  ggtitle("Our First Amazing Plot\nwith 2 Variables!!") + 
  theme_light() +
...
ggsave() is hte function to save. It will save the last plot object in the screen. In the (), (filename = "name.png"). Run ggsave(filename= "testplot.png") and it'll save!!!!!!!!

ggsave(filename= "testplot.png", width = "6cm", height = "8cm")
Save while controlling for size
